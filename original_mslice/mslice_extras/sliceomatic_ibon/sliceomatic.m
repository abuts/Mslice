function sliceomatic(U1,U2,U3,S,xlabel,ylabel,zlabel,xaxis,yaxis,zaxis,clim)
...
%   - Extra modifications in order to fix 'uicontrols'. I.Bustinduy Nov 2005.  
%
% Syntax:
%   >> sliceomatic (U1, U2, U3, S, xlabel, ylabel, zlabel,...
%                                       xaxis, yaxis, zaxis, clim)
%
...
%   U2      Limits along y axis (data at linspace(u2(1),u2(2),size(S,1))
%   U3      Limits along y axis (data at linspace(u3(1),u3(2),size(S,3))
%   S       Data values
...
%   clim    Intensity limits [0, 10]
%
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%               E   X   A   M   P   L   E
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%         u1=linspace(-10, 10, 50);
%         u2=linspace(-10, 10, 30);
%         u3=linspace(-10, 10, 40);
%         [U1,U2,U3]=meshgrid(u1,u2,u3);
%         S=sin(U1) + cos(U2) +tan(U3); 
%         sliceomatic (U1, U2, U3, S, 'x', 'y', 'z','u_1','u_2', 'u_3', [-2, 2]) 
% ---------------------------------------------------------------------------
%
%
% AUTHORS: Eric Ludlam, Srikanth Nagella (eScience), Toby Perring and Ibon Bustinduy (ISIS Excitations team).
%
%
