function disp_pow(data,vx_min,vx_max,vy_min,vy_max,i_min,i_max,shad,map,linearlog);
% function disp_pow(data,vx_min,vx_max,vy_min,vy_max,i_min,i_max,shad,map);
...
% to add more postscript printers to the list under 'ColorPrint' menu go to
% line 37 

%=== if 'disp_spe' figure does not exist, create
fig=findobj('Tag','disp_pow');
if isempty(fig),
   colordef none;	% white background % modified by I.Bustinduy
   %colordef none;	% black background
   % === create a new figure with 'Tag' property 'disp_spe'
   fig=figure('Tag','disp_pow','PaperPositionMode','auto','Name','MSlice : Display multiresolution POWDER','Renderer','zbuffer');	
   ...
   oldfig=findobj('Tag','old_disp_pow'); 
   if ~isempty(oldfig),
