...
%
% Description 
%           msma display a multiresolution slice, where all the data information will be 
...
if isempty(fig),
   disp('MULTIRESOLUTION window NOT active: Open MULTIRESOLUTION first.');
   return;
end
% ===== read parameters from ControlWindow
vars=str2mat('z','vz_min','vz_max','vx_min','vx_max','bin_vx');
vars=str2mat(vars,'vy_min','vy_max','bin_vy');
...
h=findobj(fig,'Tag',['color_slider_min_value']);
   if isempty(h),
      disp(['!! Warning: object with Tag ''color_slider_min_value'' not found']);
   end
      value=get(h,'String');
      value = str2num(value);
...
h=findobj(fig,'Tag',['color_slider_max_value']);
   if isempty(h),
      disp(['!! Warning: object with Tag ''color_slider_max_value'' not found']);
   end
      value=get(h,'String');
      value = str2num(value);
...
%    if isempty(h_ma)|~ishandle(h_ma)|isempty(get(h_ma,'UserData')),
...
%-----------------------------------------
      ...
      %MA_d=multires2d(x, y, int, err, vx_min, vx_max, bin_vx, fix_x, vy_min,vy_max,bin_vy, fix_y, NLEVEL,NERROR,varargin)
   else
