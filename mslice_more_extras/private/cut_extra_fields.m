...
if (sample==1)&&~isempty(findstr(lower(OutputType),'hkl')),
    ...
    try
        ...
        data_out.hkl=zeros(length(data.det_group),length(data.en),3);
        [data_out.hkl(:,:,1),data_out.hkl(:,:,2),data_out.hkl(:,:,3)]=...
            spe2proj_df(data.emode,data.efixed,data.en,...
            data.det_theta,data.det_psi,data.psi_samp,...
            [aa 0],[bb 0],[cc 0]);
    catch
        data_out.hkl=q2rlu(sqe2samp(spe2sqe(data),data.psi_samp),data.ar,data.br,data.cr);	% (h,k,l)
    end
