...
n_dx = floor(DVX/dx)+1;
n_dy = floor(DVY/dy)+1;
dx   = DVX/(n_dx-1); % make the step precize to cover the whole range;
dy   = DVY/(n_dy-1);
...
% select the one, contribuing into the range
...
% calulete the detectors positions within the interpolation grid:
