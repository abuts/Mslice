...
!         1         2         3         4         5         6         7         8         9         0
  ...
  call cut3d_df(%val(vx_pr),%val(vy_pr),%val(vz_pr),%val(pixel_int_pr),%val(pixel_err_pr), ndet, ne,&
       ndet*ne, vx_min,vx_max,bin_vx,eps,n,vy_min,vy_max,vz_min,vz_max,%val(x_pr),&
       %val(intensity_pr),%val(error_int_pr),m,%val(perm_pr),%val(number_pix_pr),final_npixel, vvy(1),vvz(1),&
       %val(temp1),%val(temp2),%val(temp3))
  ...
  temp=mxCreatedoublematrix(final_npixel,longOne,0)
  temp_pr= mxgetpr(temp)
  call reduce_output(final_npixel,ndet*ne,%val(perm_pr),%val(temp_pr))
  call mxDestroyArray(plhs(4))
  plhs(4)=temp
  if (m .lt. n) then
     ! deal with x,intensity,error,number_pix now
     temp=mxCreatedoublematrix(longOne,m,0)
     temp_pr= mxgetpr(temp)
  ...
  call mxCopyReal8ToPtr(vvy,mxGetPr(plhs(6)),longOne)
  call mxCopyReal8ToPtr(vvz,mxGetPr(plhs(7)),longOne)
  return
...
subroutine cut3d_df(vx,vy,vz,pixel_int,pixel_err,ndet,ne,Npixels,vx_min,vx_max,bin_vx,&
  ...
  mwsize :: m, n,final_npixel,ndet, ne,Npixels
  integer pixel_index1(*), pixel_index2(*), pixel_index3(*),bin_index(n), cumm_npixels(n)
