...
if to_cut_file
    ...
    if numel(data.det_group)==1 % only one detector, put data in columns
...
if exist('i_min','var')&&~isempty(i_min)&&isnumeric(i_min),
    if (i_min<ymax),
        ...
        fprintf('Given y-range min=%g not suitable for graph.',i_min);
...
if exist('i_max','var')&&~isempty(i_max)&&isnumeric(i_max),
    if (i_max>ymin),
        ...
        fprintf('Given y-range max=%g not suitable for graph.',i_max);
