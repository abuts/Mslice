...
subroutine mexFunction(nlhs, plhs, nrhs, prhs)
  ...
  integer*8,pointer:: vx_pr, vy_pr, vz_pr, pixel_int_pr, pixel_err_pr, grid_pr
  integer*8,pointer:: x_pr, intensity_pr, error_int_pr, perm_pr, number_pix_pr
  integer*8,pointer::  temp1, temp2, temp3
  ...
  real*8 grid(7), vx_min, vx_max, bin_vx,vy_min, vy_max, vz_min, vz_max,eps , vvy, vvz
  interface
     function mxgetpr(pm)
  ...
  call cut3d_df(vx_pr,vy_pr,vz_pr,pixel_int_pr,pixel_err_pr, ndet, ne,&
       ...
       intensity_pr,error_int_pr,m,perm_pr,number_pix_pr,final_npixel, vvy,vvz,&
       temp1,temp2,temp3)
  ...
  call reduce_output(perm_pr,temp)
  ...
  if (m .lt. n) then
     ...
     call reduce_output(x_pr,temp)
     plhs(1)=temp

     temp=mxCreatedoublematrix(1,m,0)
     call mxFreeMatrix(plhs(2))
     call reduce_output(intensity_pr,temp)
     plhs(2)=temp

     temp=mxCreatedoublematrix(1,m,0)
     call reduce_output(error_int_pr,temp)
     call mxFreeMatrix(plhs(3))
     plhs(3)=temp

     temp=mxCreatedoublematrix(1,m,0)
     call mxFreeMatrix(plhs(5))
     call reduce_output(number_pix_pr,temp)
     plhs(5)=temp 
  end if
  ! now filled implicitly
...
! =============================================================================== 
subroutine cut3d_df(vx,vy,vz,pixel_int,pixel_err,ndet,ne,Npixels,vx_min,vx_max,bin_vx,eps,n,vy_min,vy_max,vz_min,vz_max,x,intensity,error_int,m,perm,number_pix,final_npixel,vvy,vvz,pixel_index1,pixel_index2,pixel_index3)
  ...
  real*8 vx(*), vy(*), vz(*), pixel_int(*), pixel_err(*), vx_min, vx_max, bin_vx, eps, vy_min, vy_max, vz_min, vz_max,x(*), intensity(*),error_int(*), perm(Npixels),number_pix(n), vvy, vvz
  ...
  if (bin_vx .gt. eps) then  ! normal binning along the vx-axis 
     do j=1,ndet
        do l=1,ne
           ...
           if ((vx(k).ge.vx_min).and.(vx(k).lt.vx_max).and.(vy(k).ge.vy_min).and.(vy(k).le.vy_max).and. (vz(k).ge.vz_min).and.(vz(k).le.vz_max).and. (pixel_int(k) .gt. -1d+30)) then ! also test if pixel is not masked  
...
subroutine reduce_output (full,reduced)
  integer*8::full(:),reduced(:)
  integer*8::m
  m=size(reduced)
