...
subroutine mexFunction(nlhs, plhs, nrhs, prhs)
  ...
  integer*8,pointer:: vx_pr, vy_pr, pixel_int_pr 
  integer*8,pointer:: pixel_err_pr, grid_pr,  temp1, temp2, temp3
  integer*8,pointer:: x_pr, intensity_pr, error_int_pr, perm_pr, number_pix_pr
  ! declare calling functions
  integer*8 mxCreateFull, mxGetM, mxGetN, mxIsNumeric !, mxGetPr,mxCalloc
  ! declare local operating variables of the interface funnction
  integer*8 ndet, ne, n, m, final_npixel
  real*8 grid(5), vx_min, vx_max, bin_vx,vy_min, vy_max, vvy, eps
  interface
     function mxgetpr(pm)
  ...
  call cut2d_df(vx_pr,vy_pr,pixel_int_pr,pixel_err_pr, ndet, ne, ndet*ne,&
       vx_min,vx_max,bin_vx,eps,n,vy_min, vy_max,x_pr,intensity_pr,error_int_pr,&
       m,perm_pr,number_pix_pr,final_npixel,vvy,temp1,temp2,temp3)
  ...
  call mxCopyReal8ToPtr(perm_pr,mxgetPr(temp),final_npixel)
  call mxFreeMatrix(plhs(4))
  plhs(4)=temp
  if (m .lt. n) then
     ! deal with x,intensity,error,number_pix now
     temp=mxCreateFull(1,m,0)
     call mxCopyReal8ToPtr(x_pr,mxGetPr(temp),m)
     call mxFreeMatrix(plhs(1))
     plhs(1)=temp
     temp=mxCreateFull(1,m,0)
     call mxCopyReal8ToPtr(intensity_pr,mxGetPr(temp),m)
     call mxFreeMatrix(plhs(2))
     plhs(2)=temp         
     temp=mxCreateFull(1,m,0)
     call mxCopyReal8ToPtr(error_int_pr,mxGetPr(temp),m)
     call mxFreeMatrix(plhs(3))
     plhs(3)=temp 
     temp=mxCreateFull(1,m,0)
     call mxCopyReal8ToPtr(number_pix_pr,mxGetPr(temp),m)
...
! =============================================================================== 
subroutine cut2d_df(vx,vy,pixel_int,pixel_err,ndet,ne,Npixels,vx_min,vx_max,bin_vx,eps,n,vy_min,vy_max,x,intensity,error_int,m,perm,number_pix,final_npixel,vvy,pixel_index1,pixel_index2,pixel_index3)
  ! declare input and output variables
  implicit NONE
  !     m = number of bins with data, 
  !     n = total number of bins	
  integer*8 m, n, ndet, ne, Npixels, pixel_index1(*), pixel_index2(*), pixel_index3(*), bin_index(n), cumm_npixels(n), final_npixel
  real*8 vx(*), vy(*), pixel_int(*), pixel_err(*),  vx_min, vx_max, bin_vx, eps, vy_min, vy_max, x(*), intensity(*),error_int(*), perm(Npixels),number_pix(n), vvy
  ...
  if (bin_vx .gt. eps) then  ! normal binning along the vx-axis 
     do j=1,ndet
        do l=1,ne
           ...
           if ((vx(k).ge.vx_min).and.(vx(k).lt.vx_max).and.(vy(k).ge.vy_min).and.(vy(k).le.vy_max).and. (pixel_int(k) .gt. -1d+30)) then ! also test if pixel is not masked  
