...
!     to calculate the resolution convoluted cross-section for the IRIS spectrometer  
!     20-Sept-1999 compiled for PC/WINNT using Matlab 5.3 and 
!     Digital Visual Fortran Professional Edition 6.0
!==============================================================================	
subroutine mexFunction(nlhs, plhs, nrhs, prhs)
  ...
  integer*8 mxCreateFull, mxGetM, mxGetN, mxIsNumeric
  ! declare local operating variables of the interface funnction
  integer*8 npixels_total,  nbins
  integer*8,parameter:: npixels_total_max=1e5, nbins_max=1e3
  real*8:: p(15),fit_pars(2), spec_pars(5), npixels(nbins_max),&
       pixels(3,npixels_total_max),efixed, psi_samp, ar(3), br(3),&
       cr(3), ub(3,3),  y(nbins_max), error(nbins_max)
  interface
     function mxgetpr(pm)
  ...
  if (nrhs .ne. 11) then
     ...
     call mexErrMsgTxt('Two outputs (y,error) required.')
  end if
  ...
  if (npixels_total .gt. npixels_total_max) then
     call mexErrMsgTxt ('No of pixels exceeds npixels_total_max. Recompile code.')
  end if
  ! === par 6 = efixed(1,1)
  ...
  ! === par 8 = ar(1,3) lattice parameters as a (1,3) vector
  ...
  ! === Get parameter values and pixel information
  ...
  ! === Call computational subroutine ms_iris
  call ms_iris(y,error,p,fit_pars,spec_pars,npixels,nbins,&
       pixels,npixels_total,efixed,psi_samp,ar,br,cr,ub)

  ! === Create matrices for the return arguments
  plhs(1)=mxCreateFull(nbins,1,0)
  plhs(2)=mxCreateFull(nbins,1,0)

  ! === Load the output into two MATLAB arrays
...
! =================================================================================
! FORTRAN code to do the average the cross-section over the MC distribution of 
! points over the resolution volume and bin the results along a one-dimensional cut
! ================================================================================= 
subroutine ms_iris(y,error,p,fit_pars,spec_pars,npixels,nbins, pixels,npixels_total,efixed,psi_samp,ar,br,cr,ub)
  ! declare input and output variables
  implicit NONE
  !     npixels_total     ! actual total number of pixels
  !     nbins             ! actual total number of bins
  integer npixels_total, nbins, i, j, bin, pixels_in_bin, nmc, icross, seed
  double precision  y(nbins), error(nbins), p(15), spec_pars(5), pixels(3,npixels_total), efixed,&
       psi_samp, ar(3), br(3), cr(3), ub(3,3),th_h, th_v, psi_h, psi_v, dt, vo, factor, L1, &
       kix, kiy, kiz, kfx, kfy, kfz, Qx, Qy, Qz, th, thx, thy,  energy_centre, de, en, ki, kf, fit_pars(2), npixels(nbins),&
       ave_cs, err_cs
  ...
  ! === simulate scattering for each pixel
  do i=1,npixels_total 
     ...
     do j=1,nmc
        ! === generate NMC monte carlo scattering events, select a final energy froim the finite energy bin
        ! ===       energy(i)=pixels(1,i)     !  energy value (meV) of each pixel
        ! ===       energy_bin(i)=pixels(2,i) !  energy bin (meV) per pixel
        energy_centre=pixels(1,i)+(ran(seed)-0.5d0)*pixels(2,i)	! [meV]
        ! === calculate energy resulution fwhm [meV] for this energy
        ! === (~ constant over the small energy_bin(i) length) 
        de=dt*2.0d0*vo/L1*sqrt((energy_centre+efixed)**3/efixed) ! [meV]
        ! === generate a scattering event (normal distribution) centred around the nominal energy value and fwhm de
        en=energy_centre+(de/factor)*randn(seed)         ! [meV]
        ! === calculate ki and kf for this scattering event
        ki=sqrt((efixed+en)/2.072d0) !	[Angs^-1]
        kf=sqrt(efixed/2.072d0)	 !      [Angs^-1]
        ! === generate an angular orientation for ki
        thx=th_h*randn(seed)
        thy=th_v*randn(seed)
        ...
        kix=ki*cos(th)
        ! === generate an angular orientation for kf
        ...
        ! ===       theta(i)=pixels(3,i)    ! total scattering angle  (radians) for each pixel
        ...
        ! === calculate cross-section for this scattering event
        ! === and store result in a temporary variable Qx 
     ...
     if (nmc .gt. 1) then 
        err_cs=sqrt(abs(err_cs-ave_cs**2))* sqrt(1.0d0/(dfloat(nmc)-1.0d0))
     else
        err_cs=0.0d0
     end if
     ...
     ! === calculate average per bin and move on to the next bin 
  ...
  return
end subroutine ms_iris

! ===============================================================================
! Fortran code to generate a random number with a normal distribution 
! from b. Jansson, Random Number Generators, p177
! ===============================================================================
...
! ===============================================================================
! Fortran code with cycloidal dispersion relation appropriate for Cs2CuCl4
! ===============================================================================
subroutine cs2cucl4(A,B,C,D,h,k,l,J,Jp)
  implicit NONE
  double precision A,B,C,D,h,k,l,J,Jp,q,pi, gam, gamp, gamp_kpq, gamp_kmq, theta, spin
  pi=4.0d0*atan(1.0d0)
  ...
  theta=2.0d0*pi*q
  spin=1/2.0d0
  A=-spin*J*(2.0d0*cos(theta)-(cos(theta)+1.0d0)*gam)+ 4.0d0*spin*Jp*cos(theta/2.0d0)
  B=2.0d0*spin*Jp*(gamp-(gamp_kpq+gamp_kmq)/2.0d0)
  C=spin*J*(cos(theta)-1.0d0)*gam
  D=-2.0d0*spin*Jp*(gamp+(gamp_kpq+gamp_kmq)/2.0d0)
  return
end subroutine cs2cucl4

! ===============================================================================
! Fortran code with fourier transforms of exchange interaction for an anisotropic 
! triangular lattice relevant to Cs2CuCl4 
! ===============================================================================
...
subroutine uv_2dtr(u,v,wq,J,Jp,qk,ql)
  implicit NONE
  double precision u, v, wq, J, Jp, qk, ql, z, atanh, pi, Q, a, b, Ak, Bk, thk, jj
  external jj 
  pi=4.0d0*atan(1.0d0)       
  ...
  b=(jj(J,Jp,Q+qk,ql)+jj(J,Jp,Q-qk,-ql))/2.0d0-jj(J,Jp,Q,0.0d0) 
  Ak=(a+b)/2.0d0
  Bk=(a-b)/2.0d0
  ...
  v=sinh(thk) 
  return
end subroutine uv_2dtr

! ===============================================================================
! Fortran code equivalent to the floor function in Matlab 
! ===============================================================================
...
! ===============================================================================
! Fortran code equivalent to the ceil function in Matlab 
! ===============================================================================
...
! ===============================================================================
! Fortran code to reduce a number x to the interval [x1,x2)
! ===============================================================================
...
! ===============================================================================
! Fortran code to return global index in a table, 0 means out of table range 
! ===============================================================================
...
! ===============================================================================
! Fortran code to calculate the complex gamma function 
! ===============================================================================
!     ALGORITHM 404 COLLECTED ALGORITHMS FROM ACM.
!     ALGORITHM APPEARED IN COMM. ACM, VOL. 14, NO. 01,
!     P. 048.
!     downloaded from http://www.netlib.org/toms/ 05-Nov-1999  
...
60 T = DCMPLX(X,Y)
  ...
  SUM = (T-(0.5d0,0.0d0))*CDLOG(T)- T+DCMPLX(0.5d0*DLOG(2.0d0*3.14159265358979d0),0.0d0)
  J = 1
70 TERM = C(J)/DEN
...
130  CGAMMA = CDEXP(SUM)
     RETURN
900  FORMAT(1X,2E14.7,10X,49H ARGUMENT OF GAMMA FUNCTION IS TOO CLOSE TO A POLE)
910    FORMAT(44H ERROR - STIRLING*S SERIES HAS NOT CONVERGED/14X,4HZ = , 2E14.7)
     END FUNCTION IS

     ! ===============================================================================
     ! Fortran code to calculate function rho(x)=G(eta-ix)/G(1-eta-ix)
     ! where G is the complex gamma function and eta=1/4 
     ! ===============================================================================
     ...
     ! ===============================================================================
     ! Fortran code with the neutron cross-section definition ms_sqw
     ! ===============================================================================
     double precision function ms_sqw(icross,p,Qx,Qy,Qz,en, twotheta,ar,br,cr)
       implicit NONE
       integer icross
       double precision p(15), Qx, Qy, Qz, en, twotheta, ar(3), br(3),  cr(3), A_sw, J,&
            dE, Jp, A_inc, sig_inc, cen, spin, temp, q, pi, eps, bose, modQ, fQ, t(7), w1, w2, nu,&
            deltas, bigq,A, B, C, D, theta, wdisp, alpha, deltaq, x, y, table(12)
       !=========================================================
       ! Define variables to deal with the one magnon and two-magnon cross-section tables  
       !========================================================= 
       integer nevents_1m_in, nevents_1m_out, nevents_2m,  maxsize_table, i, i1, index 
       character*40 filename
       external index
       double precision reduce, absnorm_1m_in, absnorm_1m_out, absnorm_2m
       external reduce
       parameter (maxsize_table=500012)
       double precision table_1m_in(maxsize_table),  table_1m_out(maxsize_table), table_2m(maxsize_table)
       logical read_tables /.FALSE./

       data eps /1.0d-5/, spin /0.5d0/, t /0.0232d0,34.969d0,0.4023d0,11.564d0,0.5882d0,3.843d0,-0.0137d0/
       ...
       ! === initialize variables and calculate Bose factor + form factor
       pi=4.0d0*atan(1.0d0)   
       ...
       ! === [Qx,Qy,Qz]=[h,k,l] in rlu  ar,br,cr (1,3)(Angs^-1) expressed in a rectangular coordinate system
       ...
       ! === calculate polarization factor, temp is a temporary variable
       ...
       if (icross .eq. 1) then 
          ...
          ! === calculate dispersion relation wQ_{xx} do not include polarisation fcator 
          call cs2cucl4(A,B,C,D,Qx,Qy,Qz,J,Jp)
          wdisp=sqrt((A+B)**2-(C+D)**2)
          temp=A_sw*spin*(A+B+C+D)/(wdisp+dE/1000.0d0)/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))
          ...
          wdisp=sqrt((A-B)**2-(C-D)**2)
          temp=temp+p(12)*spin*(A-B-C+D)/(wdisp+dE/1000.0d0)/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))

          ! === calculate dispersion relation for S_{yy} and S_{zz} : w2(Q+q) do not include polarization factor 
          call cs2cucl4(A,B,C,D,Qx,Qy+q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          temp=temp+p(13)*spin*(A-B-C+D)/(wdisp+dE/1000.0d0)/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))
          ...
          call cs2cucl4(A,B,C,D,Qx,Qy,Qz,J,Jp)
          wdisp=sqrt((A+B)**2-(C+D)**2)
          temp=A_sw*spin*(sin(alpha)**2)*(A+B+C+D)/(wdisp+dE/1000.0d0)/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))

          ! === calculate dispersion relation for S_{yy}=S_{zz} : w2(Q-q) include polarization factor (1+cos(alpha)**2)
          call cs2cucl4(A,B,C,D,Qx,Qy-q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          temp=temp+A_sw*p(12)*spin/4.0d0*(1.0d0+cos(alpha)**2)*(A-B-C+D)/(wdisp+dE/1000.0d0)/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))

          ! === calculate dispersion relation for S_{yy}=S_{zz} : w2(Q+q) include polarization factor (1+cos(alpha)**2)  
          call cs2cucl4(A,B,C,D,Qx,Qy+q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          temp=temp+A_sw*p(13)*spin/4.0d0*(1.0d0+cos(alpha)**2)*(A-B-C+D)/(wdisp+dE/1000.0d0)/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=A_sw*spin/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          else
             temp=0.0d0
          end if
          ...
          wdisp=sqrt((A-B)**2-(C-D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(12)*spin/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          wdisp=sqrt((A-B)**2-(C-D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(13)*spin/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          wdisp=sqrt((A+B)**2-(C+D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=A_sw*spin*(sin(alpha)**2)/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          else
             temp=0.0d0
          end if

          ! === calculate dispersion relation for S_{yy} and S_{zz} : w2(Q-q) include polarization factor (1+cos(alpha)**2)
          call cs2cucl4(A,B,C,D,Qx,Qy-q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+A_sw*spin*p(12)*(1.0d0+cos(alpha)**2)/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ! === calculate dispersion relation for S_{yy} and S_{zz} : w2(Q+q) include polarization factor (1+cos(alpha)**2)  
          call cs2cucl4(A,B,C,D,Qx,Qy+q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+A_sw*spin*p(13)*(1.0d0+cos(alpha)**2)/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          call cs2cucl4(A,B,C,D,Qx,Qy-q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          temp=temp+p(12)*spin/4.0d0*(1.0d0+cos(alpha)**2)*(A-B-C+D)/(wdisp+dE/100.0d0)*1.0d0/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))

          ! === calculate dispersion relation for S_{yy} and S_{zz} : w2(Q+q) include polarization factor (1+cos(alpha)**2)  
          call cs2cucl4(A,B,C,D,Qx,Qy+q,Qz,J,Jp)
          wdisp=sqrt((A-B)**2-(C-D)**2)
          temp=temp+p(13)*spin/4.0d0*(1.0d0+cos(alpha)**2)*(A-B-C+D)/(wdisp+dE/100.0d0)*1.0d0/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))
          ...
          wdisp=sqrt((A-B)**2-(C-D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(12)*A_sw*spin/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          wdisp=sqrt((A-B)**2-(C-D)**2)
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(13)*A_sw*spin/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          ! === calculate dispersion relation for S_{xx}	: w1(Q) no polarization factor
          call cs2cucl4(A,B,C,D,Qx,Qy,Qz,J,Jp)
          wdisp=sqrt((A+B)**2-(C+D)**2)
          ...
          if ((w2 .gt. 0.0d0) .and. (w2 .le. (1.0d0-2.0d0*deltaq))) then 
             ...
             w2=2.0d0*w1*p(14)* abs(sin((2.0d0-w2)*pi/(1.0d0+2.0d0*deltaq)))
          end if
          ! in case upper boundary w2 goes under the main dispersive mode put w2 just above wdisp 
          if (w2 .le. wdisp) then 
             w2=wdisp+2.355d0*dE
          ...
          ! === calculate dispersion relation for S_{xx}	: w1(Q) do not with polarization factor 
          call cs2cucl4(A,B,C,D,Qx,Qy,Qz,J,Jp)
          wdisp=sqrt((A+B)**2-(C+D)**2)
          w2=2.0d0*w1*p(14)*abs(sin(pi*Qy)) 
          ! take as the continuum upper boundary the Muller boundary for the maximum zone boundary energy
          ...
          ! === calculate dispersion relation for S_{xx}	: w1(Q) with polarization factor (sin(alpha)**2)
          call cs2cucl4(A,B,C,D,Qx,Qy,Qz,J,Jp)
          wdisp=sqrt((A+B)**2-(C+D)**2)
          ! take as the continuum upper boundary the Muller boundary for the maximum zone boundary energy
          w2=2.0d0*w1*p(14)*abs(sin(pi*Qy)) 
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(12)*A_sw*spin*(1.0d0+cos(alpha)**2)/2.0d0/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2) 
             ! 13May00 introduce factor/2 to have observed scattering same as before       
          end if
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(13)*A_sw*spin*(1.0d0+cos(alpha)**2)/2.0d0 /sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
             ! 13May00 introduce factor/2 to have observed scattering same as before
          end if
          ...
          ! === calculate dispersion relation for S_{xx}	: w1(Q)
          call cs2cucl4(A,B,C,D,Qx,Qy,Qz,J,Jp)
          wdisp=sqrt((A+B)**2-(C+D)**2)
          ...
          ! === assume zone-boundary energy is J and displace q values by Deltaq long the chain direction 
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(12)*A_sw*spin*(1.0d0+cos(alpha)**2)/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(13)*A_sw*spin*(1.0d0+cos(alpha)**2)/sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
          ...
          if (.not. read_tables) then
             ...
             if (i1 .ne. i) then
                call mexErrMsgTxt ('Tables 2m and 1m_in of different sizes.')
             end if
             ...
             absnorm_1m_in=1.0d0/table_1m_in(3)/table_1m_in(7)/     table_1m_in(11)/dfloat(nevents_1m_in)
             absnorm_1m_out=1.0d0/table_1m_out(3)/table_1m_out(7)/    table_1m_out(11)/dfloat(nevents_1m_out)
             absnorm_2m=1.0d0/table_2m(3)/table_2m(7)/     table_2m(11)/dfloat(nevents_2m)
             do i=13,i1
          ...
          if (i .gt. 0) then 
             temp=temp + spin*(1.0d0+ cos(alpha)**2)*p(12)*p(1)      *absnorm_1m_in*table_1m_in(i)/4.0d0 ! (k-Q,w) first
          end if
          x=(Qz+(Qy+bigq))/2.0d0+1.0d0
          y=(Qz-(Qy+bigq))/2.0d0+1.0d0
          i=index(x,y,en/p(2),table)
          if (i .gt. 0) then 
             temp = temp+spin*(1.0d0+ cos(alpha)**2)*p(12)*p(1)      *absnorm_1m_in*table_1m_in(i)/4.0d0 ! add (k+Q,w)
          end if
          ...
          i=index(x,y,en/p(2),table)
          if (i .gt. 0) then 
             temp = temp + spin*(sin(alpha)**2)*p(1)*        absnorm_1m_out*table_1m_out(i) ! (k,w) component
          end if

          ! === calculate S(k-Q,w) and S(k+Q,w) 2-magnon (all in-plane polarization)
          absnorm_2m=(((1+deltas)*deltas +p(4)**2)/         (0.5d0*(spin-deltas-p(4)**2/(1+2*deltas))))
          do i=1,12
             table(i)=table_2m(i)
          end do
          x=(Qz+(Qy-bigq))/2.0d0+1.0d0
          ...
          i=index(x,y,en/p(2),table)
          if (i .gt. 0) then 
             temp = temp+spin*(1.0d0+ cos(alpha)**2)*p(13)       *absnorm_2m*table_2m(i)/4.0d0  ! (k-Q,w) first
          end if
          x=(Qz+(Qy+bigq))/2.0d0+1.0d0
          y=(Qz-(Qy+bigq))/2.0d0+1.0d0
          i=index(x,y,en/p(2),table)
          if (i .gt. 0) then 
             temp = temp+spin*(1.0d0+ cos(alpha)**2)*p(13)        *absnorm_2m*table_2m(i)/4.0d0 ! add (k+Q,w)
          end if
          ...
          ! take as the continuum upper boundary the Muller boundary for the maximum zone boundary energy
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=A_sw*spin/exp(p(3)*log(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2))
          else
             temp=0.0d0
          end if
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(12)*A_sw*spin/     exp(p(3)*log(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2))
          end if
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+p(13)*A_sw*spin/     exp(p(3)*log(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2))
          end if
          ...
          ! === calculate dispersion relation for S_{xx}(k,w) : w(k) and include polarization factor (sin(alpha))**2
          call uv_2dtr(A,B,wdisp,J,Jp,Qy,Qz)  ! here u=A, v=B
          temp=A_sw*spin*(sin(alpha)**2)*abs(A-B)**2/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))

          ! === calculate dispersion relation for S_{yy} and S_{zz}(k,w): w(k-Q) include polarization factor (1+cos(alpha)**2)
          bigq=1.0d0-q           ! Q in the triangular lattice formalism         
          call uv_2dtr(A,B,wdisp,J,Jp,Qy-bigq,Qz)  ! here u=A, v=B
          temp=temp+A_sw*p(12)*spin/4.0d0*(1.0d0+cos(alpha)**2)*abs(A+B)**2/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))

          ! === calculate dispersion relation for S_{yy} and S_{zz}(k,w): w(k+Q) include polarization factor (1+cos(alpha)**2)  
          call uv_2dtr(A,B,wdisp,J,Jp,Qy+bigq,Qz)  ! here u=A, v=B
          temp=temp+A_sw*p(13)*spin/4.0d0*(1.0d0+cos(alpha)**2)*abs(A+B)**2/(sqrt(2.0d0*pi)*dE)*exp(-(en-wdisp-cen)**2/(2.0d0*dE**2))
          ...
          if (((en-cen) .gt. wdisp) .and. ((en-cen) .le. w2)) then 
             temp=temp+A_sw*spin /sqrt(abs((en-cen)**2-wdisp**2)+(dE*2.355d0)**2)
          end if
       ...
       temp=(fQ**2)*bose*temp+A_inc/sqrt(2.0d0*pi)/sig_inc*exp(-(en-cen)**2/(2.0d0*sig_inc**2))+p(9)
       ...
       ms_sqw=temp
       return
     end function ms_sqw ! function ms_sqw
