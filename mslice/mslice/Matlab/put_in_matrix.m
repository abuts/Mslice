...
if exist('flip_command','var')&~isempty(flip_command)&ischar(flip_command),
   for i=1:size(flip_command,1)
       ...
       % create a string, then function handle, then use that 
       flip_commandstr = deblank(flip_command(i,:));
       flip_commandstr = str2func(flip_commandstr);
      int=flip_commandstr(int);
      err=flip_commandstr(err);
