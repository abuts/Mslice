...
for i=1:length(weight),
  ...
  % index=~((data.S(:,1)<=nulldata)|isnan(data.S(:,1))|isinf(data.S(:,1)));	% (ndet,1) is 1 where pixel is data and 0 where detector has 'nulldata' in current data set
   index=isnan(data.S(:,1));	% 
