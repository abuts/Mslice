...
if algorithm==1,
   ...
   grid=[vx_min vx_max bin_vx vy_min vy_max bin_vy vz_min vz_max];	% 2d grid information (1,8)
	...
	[xav,yav,intensity,error_int,npix,detno,epsbin,xpix,ypix,pix_int,pix_err]= ...
                  slice_df_full(data.v(:,:,x),data.v(:,:,y),data.v(:,:,z),data.S,data.ERR,grid);
   clear mex slice_df_full ; 	% ensures clearing up of memory after binning 
   ...
   if length(data.en) <= 1
       slice_d=[];
       slice_full=[];
       disp('Insufficient information to write complete slice:')
       disp('   only one energy bin in SPE file')
   end
   
   eps_lo = data.en(1);
   deps = data.en(2)-data.en(1);
   slice_full.header_info = [size(xav,2) size(xav,1) vx_min vy_min bin_vx bin_vy];
   slice_full.xav = xav;
   slice_full.yav = yav;
   slice_full.intensity = intensity;
   slice_full.error_int = error_int;
   slice_full.npix = npix;
   slice_full.detno = data.det_group(detno); % as masked detectors were removed
   slice_full.epsbin = eps_lo + deps.*(epsbin-1);
   slice_full.deps = deps;
   slice_full.xpix = xpix;
   slice_full.ypix = ypix;
   slice_full.pix_int = pix_int;
   slice_full.pix_err = pix_err;
...
% === write further information to slice_full structure
slice_full.title  = slice_d.title;
