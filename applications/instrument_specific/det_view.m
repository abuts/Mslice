function plot_sum(spec_data,i_min,i_max)
% function plot_sum(spec_data,i_min,i_max)

% === return spec_data=[spec_number intensity error] (nspectra,3)
if isempty(spec_data),
   disp(sprintf('Empty spectrum data in file \n%s',sum_filename));
   return;
end
...
if isempty(fig),	% new figure
   ...
   fig=figure('Tag','det_view','PaperPositionMode','auto');	
...
if exist('i_max','var')&~isempty(i_max)&isnumeric(i_max)&prod(size(i_max))>=1,
   ...
   i_max=min([max([int_e(:); int_w(:)]) max([int_n(:); int_s(:)])]);
end 
...
h=title({'Colour plot of integrated white vanadium intensities over the HET PSD banks', ...
      avoidtex(sum_filename)});
YLim=get(gca,'YLim');
pos=get(h,'Position');
