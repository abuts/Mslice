...
elseif(samp==1)&&(analmode==3) % powder remap
    ...
    data.u=[u1;u2];
    data.range = range;
    [data,range,range_changed]=rebin_cryst_img_to_rings(data);
    data=calcprojpowder(data);
    if range_changed
        range_vars={'polar_min','polar_max','polar_delta'};
        for i=1:numel(range_vars)
            if isfield(range,[range_vars{i},'_changed'])
                ms_setvalue(range_vars{i},range.(range_vars{i})*180/pi,'highlight');
            end
        end
    end
