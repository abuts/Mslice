...
elseif samp~=2,
   ...
   error('Unknown operational mode in mslice with sample type %g',samp);
   return;
end
...
elseif (samp==2)||((samp==1)&&(analmode==2))|| ...
       ((samp==1)&&(analmode==1)&&(psd==2))	% powder or single crystal + non-PSD detectors
    if x==1,
   	    y=2;
	else 
  		y=1;
