function [pos,pos_u1step,pos_u2step,mslice_gui] = msui_powder_display(pos,h_root,oneline,interlines,white,coltab)
% Interface to Powder: Display projections
% Outputs:
...
if exist(mslice_gui,this_name)
    ...
    pos(3)=width;
    pos(2)=pos(2)+3*height;
    [pos_u1step,pos_u2step] =get_tab_position(pos,oneline,width,interlines);    
    return;
end
...
hb{9}=uicontrol('Parent',h_root,'Style','edit','Tag','ms_disp_vy_max',...
  	...
  	'HorizontalAlignment','left',...
  	'BackgroundColor',white,'Visible','off');
    % provisional position of the step tab: --> calculated above
...
mslice_gui=add(mslice_gui,add_handles(powder_slice,hb));

function [pos1,pos2]=get_tab_position(pos,oneline,width,interlines)
pos1=pos+[5*pos(3)+width 0 0 interlines];
pos2=pos1 - oneline;     
