...
%
% Input:
% obj              -- initialized instance of config_stor_msl class
...
if isa(class_or_name_to_restore,'config_bas_msl')
...
elseif istext(class_or_name_to_restore)
    ...
    error('HERBERT:config_stor_msl:invalid_argument',...
        'Config class %s has to be a child of the config_bas_msl class or the name of such class', ...
...
if numel(varargin) < nargout
    error('HERBERT:config_stor_msl:runtime_error',...
...
for i=1:nfields
    if isfield(config_data,varargin{i})
        ...
        if ~ismember(varargin{i},class_inst.mem_only_prop_list)
            warning('HERBERT:config_stor_msl:default_field_value',...
