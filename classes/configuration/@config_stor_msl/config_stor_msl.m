classdef config_stor_msl < handle
    ...
    methods(Static)
        ...
        function set_config_folder(config_folder_name)
            ...
            if ~ischar(config_folder_name)
                error('HERBERT:config_stor_msl:invalid_argument',...
    ...
    methods
        function store_config(obj,config_class,varargin)
            ...
            if ~ok
                error('HERBERT:config_stor_msl:invalid_argument',mess);
            end
        ...
        function   varargout = get_value(obj,class_name_or_inst,varargin)
            ...
            %Usage:
            %[val1,val2,...] =
            %        config_stor_msl.instance().get_config_field(config_class,
    ...
    function   config_data=get_config(this,class_to_restore)
        ...
        %Usage:
        %
            % obj = conifg_store.instance().restore_config(mslice_config)
        %       unusual instance of Herbert config, with modified
        %       defaults. Should not be used
        %
            % [use_mex,log_level]=conifg_store.instance().restore_config(mslice_config,...
        %                     'use_mex','log_level')
        ...
        % file
        if read_from_file
            if ~isa(class_to_restore,'config_bas_msl') % it mast be char
    ...
    function set_config_path(obj,new_path,varargin)
        ...
        % usage:
            % config_stor_msl.instance(new_path,['-clean']);
        %
        % Inputs:
        ...
        if ~(ischar(new_path)||isstring(new_path))
            error('HERBERT:config_stor_msl:invalid_argiment',...
