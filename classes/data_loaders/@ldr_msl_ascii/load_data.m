...
use_mex=config_stor_msl.instance().get_value('mslice_config','use_mex');
if use_mex
    try
        [S,ERR,en] = get_ascii_file(file_name ,'spe');
    catch err
		force_mex=config_stor_msl.instance().get_value('mslice_config','force_mex');
        if ~force_mex
            ...
            set(mslice_config,'use_mex',false);  % don't use Herbert C++ routines from now on
            use_mex=false;
        else
