classdef ldr_msl_ascii < a_msl_loader
    ...
    methods(Static)
        ...
        function descr=get_file_description()
            ...
            % loader.
            ext = ldr_msl_ascii.get_file_extension();
        ...
        function [ok,fh] = can_load(file_name)
            ...
            % Output:
            %
            % ok   -- True if the file can be processed by the ldr_msl_ascii
            ...
            [ndet,en,full_file_name]=ldr_msl_ascii.get_data_info(file_name);
        ...
        function [ndet,en,full_file_name]=get_data_info(file_name)
            ...
            %
            % >> [ndet,en,full_file_name] = ldr_msl_ascii.get_data_info(filename)
            %
            % where:
    ...
    methods
        function obj = ldr_msl_ascii(full_spe_file_name,varargin)
            ...
            %
            % usage:
            %>> loader =ldr_msl_ascii();
            %>> loader =ldr_msl_ascii(spe_file)
            %>> loader =ldr_msl_ascii(spe_file,par_file)
            %
            % where:
