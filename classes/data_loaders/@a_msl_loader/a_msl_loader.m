classdef a_msl_loader < a_detpar_loader_interface
    ...
    % are defined in asciipar_msl_ldr.
    %
    ...
    properties(Constant,Access=protected)
        fext_to_parloader_map_ = containers.Map({'.par','.phx','.nxspe'},...
            {asciipar_msl_ldr(),asciipar_msl_ldr(),nxspepar_loader()});
    end
    ...
    methods(Abstract)
        ...
        %   this =the_loader@a_msl_loader(par_file);
    ...
    methods
        % constructor;
        function obj=a_msl_loader(varargin)
            ...
            %
            if(nargin>0)
                if isa(varargin{1},'a_msl_loader')
                    obj = varargin{1};
                else
        ...
        function [ok,mess,ndet,en]=is_loader_valid(this)
            ...
            % be invoked for a_msl_loader, as uses abstract class methods,
    ...
    methods(Access=protected)
        ...
        function obj = set_par_file_name(obj,par_f_name)
            ...
            ldr = a_msl_loader.fext_to_parloader_map_(lower(fext));
