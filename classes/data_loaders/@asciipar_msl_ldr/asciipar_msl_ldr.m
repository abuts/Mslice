classdef asciipar_msl_ldr < a_detpar_loader_interface
    ...
    methods
        % constructor;
        function this=asciipar_msl_ldr(varargin)
            % initiate the list of the fields this loader defines
            %>>Accepts:
            %   default empty constructor:
            %>>this=asciipar_msl_ldr();
            %   constructor, which specifies par file name:
            %>>this=asciipar_msl_ldr(par_file_name);
            %   copy constructor:
            %>>this=asciipar_msl_ldr(other_loader);
            %
            if nargin>0
                if ischar(varargin{1})
                    this.par_file_name = varargin{1};
                elseif isa(varargin{1},'asciipar_msl_ldr')
                    this=varargin{1};
                else
                    error('ASCIIPAR_LOADER:invalid_argument','asciipar_msl_ldr constructor argument can be only par file name or other loader')
                end
            else
            end
        end
    ...
    methods(Access=protected)
        ...
        function obj=set_par_file_name(obj,par_f_name)
            ...
            if isempty(par_f_name)
                ...
                if ~ok
                    error('HERBERT:asciipar_msl_ldr:invalid_argument',mess);
                end
                if ~strcmp(obj.par_file_name_,f_name)
                    obj.par_file_name_= f_name;
                    obj.n_det_in_par_ = asciipar_msl_ldr.get_par_info(f_name);
                    obj.det_par_=[];
                end
            end
        end
